...
func DeployCmd(noMetrics *bool) *cobra.Command {
	...
	deployMinio := &cobra.Command{
		...
		Run: pkgcobra.RunBoundedArgs(pkgcobra.Bounds{Min: 4, Max: 4}, func(args []string) (retErr error) {
			...
			manifest := &bytes.Buffer{}
			err := assets.WriteMinioAssets(manifest, opts, hostPath, args[0], args[1], args[2], args[3], isSecure)
			if err != nil {
				return err
			}
			return maybeKcCreate(dryRun, manifest)
	...
	deployMinio.Flags().BoolVarP(&isSecure, "secure", "s", false, "Enable secure access to Minio server.")
