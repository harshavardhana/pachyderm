...
//   isSecure - Is a secure connection.
func MinioSecret(bucket string, id string, secret string, endpoint string, isSecure bool) *api.Secret {
	secure := func() string {
		if isSecure {
			return "1"
		}
...
func WriteMinioAssets(w io.Writer, opts *AssetOpts, hostPath string, bucket string, id string,
	secret string, endpoint string, isSecure bool) error {
	if err := WriteAssets(w, opts, minioBackend, nil, 1 /* = volume size (gb) */, hostPath); err != nil {
		return err
	}
	encoder := codec.NewEncoder(w, jsonEncoderHandle)
	MinioSecret(bucket, id, secret, endpoint, isSecure).CodecEncodeSelf(encoder)
	fmt.Fprintf(w, "\n")
	return nil
}
