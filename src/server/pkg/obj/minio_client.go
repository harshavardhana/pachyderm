...
func newMinioClient(endpoint, bucket, id, secret string, isSecure bool) (*minioClient, error) {
	mclient, err := minio.New(endpoint, id, secret, isSecure)
	if err != nil {
		return nil, err
	}
	return &minioClient{
...
func (c *minioClient) Walk(name string, fn func(name string) error) error {
	...
	defer close(doneCh)
	for objInfo := range c.ListObjectsV2(c.bucket, name, isRecursive, doneCh) {
		if objInfo.Err != nil {
			return objInfo.Err
		}
